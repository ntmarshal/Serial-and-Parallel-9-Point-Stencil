...
int main(int argc, char** argv){
    ...
    if (argc != 5) {
        printf("Usage: mpirun -np <p> ./mpi-stencil-2d <num iterations> <input file> <output file> <num_threads>\n");
        MPI_Finalize();
        exit(1);
    }
    ...
    for(int k = 0; k < maxIterations; k++){
        ...
        if(p > 1){
        parallel_stencil((double**)x,(double**)xNew,BLOCK_SIZE_HALO(id,p,rows),cols,numThreads);
        } else {
        parallel_stencil((double**)x,(double**)xNew,rows,cols,numThreads);
