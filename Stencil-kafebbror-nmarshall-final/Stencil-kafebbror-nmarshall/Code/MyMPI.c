      ...
      MPI_Comm     comm)     /* IN - Communicator */
{
   ...
   local_cols = BLOCK_SIZE(id,p,*n) + 1;
    ...
    char* filename)
{
   ...
   if (!id) {
    ...
    fwrite(&n,sizeof(int),1,fp);
      fwrite(a[0], sizeof(dtype),local_rows*n, fp);
      if (p > 1) {
         datum_size = get_size (dtype);
         max_block_size = BLOCK_SIZE(p-1,p,m);
         bstorage = my_malloc (id,
         ...
         for (i = 1; i < p; i++) {
            ...
            fwrite(b[0], sizeof(dtype), BLOCK_SIZE(i,p,m)*n, fp);
         }
         free (b);
         free (bstorage);
         fclose(fp);
   ...
   MPI_Comm     comm)     /* IN - Communicator */
{
   ...
   *storage = (void *) my_malloc (id,BLOCK_SIZE_HALO(id,p,*m) * *n * sizeof(double));
   *subs = (void **) my_malloc (id, BLOCK_SIZE_HALO(id,p,*m) * PTR_SIZE);
   ...
   //Set all values to -1.0 to start
   ...
   if(p > 1){
        if (id == (p-1)) {
            //Send values to other process from p-1
            for (i = 0; i < p-1; i++) {
                fread(((double**)(*subs))[1], datum_size,BLOCK_SIZE(i,p,*m) * *n, infileptr);
                MPI_Send(((double**)(*subs))[1], BLOCK_SIZE(i,p,*m) * *n, dtype, i, DATA_MSG, comm);
            }
            //Read process 1 data into row 1
            fread(((double**)(*subs))[1], datum_size, local_rows * *n,infileptr);

            fclose(infileptr);
    } else {
        if ( id == 0 ){
            //If id = 0 read the data into row 0
            MPI_Recv(((double**)(*subs))[0], BLOCK_SIZE(id,p,*m) * *n, dtype, p-1,DATA_MSG, comm, &status);

    } else {
            //else read the data into row 1
            MPI_Recv(((double**)(*subs))[1],BLOCK_SIZE(id,p,*m) * *n, dtype, p-1, DATA_MSG, comm, &status);


      }
    }
   } else {
            //if there is only one process read all the data into the start of the array
   ...
   MPI_Comm comm)       /* IN - Communicator */
{
   ...
   if (!id) {
       //Print process 0 matrix
      print_submatrix (a, dtype, local_rows, n);

      //If there is more than one process allocate enough space for data and halo rows
      if (p > 1) {
         datum_size = get_size (dtype);
        ...
        //For every other process, store the data recieved into matrix b and print
         for (i = 1; i < p; i++) {
            MPI_Send (&prompt, 1, MPI_INT, i, PROMPT_MSG,
               MPI_COMM_WORLD);

            MPI_Recv (bstorage, BLOCK_SIZE_HALO(i,p,m)*n, dtype,i, RESPONSE_MSG, MPI_COMM_WORLD, &status);
      ...
      putchar ('\n');
       ...
       //Send all the data and halo rows to process 0
      MPI_Recv (&prompt, 1, MPI_INT, 0, PROMPT_MSG,
         MPI_COMM_WORLD, &status);
      MPI_Send (a[0], BLOCK_SIZE_HALO(id,p,m) * n, dtype, 0, RESPONSE_MSG,MPI_COMM_WORLD);
   }
}
    ...
    char* filename)
{
   ...
   if (!id) {
    ...
    //Let process 0 write the row and column data as well as its data rows
    fwrite(&m,sizeof(int),1,fp);
    fwrite(&n,sizeof(int),1,fp);
    fwrite(a[0], sizeof(dtype),local_rows*n, fp);
        //If there is more than 1 process, allocate space to recieve data
      if (p > 1) {
         datum_size = get_size (dtype);
         max_block_size = BLOCK_SIZE(p-1,p,m);
         bstorage = my_malloc (id,
         ...
         //Recieve data from every other process and write the data to the file
         for (i = 1; i < p; i++) {
            MPI_Send (&prompt, 1, MPI_INT, i, PROMPT_MSG,
               MPI_COMM_WORLD);
            MPI_Recv (bstorage, BLOCK_SIZE(i,p,m)*n, dtype,
       ...
       //Send data starting from row 1
      MPI_Recv (&prompt, 1, MPI_INT, 0, PROMPT_MSG,
         MPI_COMM_WORLD, &status);
      MPI_Send (a[1], local_rows * n, dtype, 0, RESPONSE_MSG,
         MPI_COMM_WORLD);
   }
...

{
    ...
    //Optain information for process
    MPI_Comm_rank (comm, &id);
    MPI_Comm_size (comm, &p);
    local_rows = BLOCK_SIZE_HALO(id,p,m);
    ...
    //Check for more than one process
    if(p > 1){
        //If id is 0, recieve into last row and send second to last row, wait for them to finish
        if(id == 0){
            MPI_Irecv(((double**)(*subs))[local_rows -1],n,dtype,id+1,99,MPI_COMM_WORLD,&reqs[0]);
            MPI_Isend(((double**)(*subs))[local_rows -2],n,dtype,id+1,99,MPI_COMM_WORLD,&reqs[1]);
            MPI_Waitall(2,reqs,stats);
            //If id is the last process, recieve into first row, send second row
        } else if (id == p-1){
            MPI_Irecv(((double**)(*subs))[0],n,dtype,id-1,99,MPI_COMM_WORLD,&reqs[0]);
            MPI_Isend(((double**)(*subs))[1],n,dtype,id-1,99,MPI_COMM_WORLD,&reqs[1]);
            MPI_Waitall(2,reqs,stats);
            //Else, recieve into first and last row, send second and second to last row
        } else {
            MPI_Irecv(((double**)(*subs))[local_rows -1],n,dtype,id+1,99,MPI_COMM_WORLD,&reqs[0]);
            MPI_Irecv(((double**)(*subs))[0],n,dtype,id-1,99,MPI_COMM_WORLD,&reqs[1]);
            MPI_Isend(((double**)(*subs))[local_rows - 2],n,dtype,id+1,99,MPI_COMM_WORLD,&reqs[2]);
            MPI_Isend(((double**)(*subs))[1],n,dtype,id-1,99,MPI_COMM_WORLD,&reqs[3]);
